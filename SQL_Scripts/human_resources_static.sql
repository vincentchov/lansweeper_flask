...
AS (
    ...
    WHERE htblticketcustomfield.fieldid
        IN (15,130,131,133,186)
    ORDER BY [TicketID],[FieldID]
)
SELECT *
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [HR Process and Personnel Related issues],[Human Resource / Personnel Issues],[Location of Issue],[Pay / Benefits Related Issues],[Recruiting / Onboarding Related Issues]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([HR Process and Personnel Related issues],[Human Resource / Personnel Issues],[Location of Issue],[Pay / Benefits Related Issues],[Recruiting / Onboarding Related Issues])
    ) p
) y
