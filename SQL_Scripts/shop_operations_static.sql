...
AS (
    ...
    WHERE htblticketcustomfield.fieldid
        IN (41,52,112,161,162,164,166)
    ORDER BY [TicketID],[FieldID]
)
SELECT *
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [Shop Locations],[This could be a safety risk],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([Shop Locations],[This could be a safety risk],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)])
    ) p
) y
