...
AS (
    ...
    WHERE htblticketcustomfield.fieldid
        IN (52,146,147,148,150,173,174,175)
    ORDER BY [TicketID],[FieldID]
)
SELECT *
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [Client / Customer Name],[Leadership / Management Issues],[Management or Project Management Issue],[Project Management Issues],[Project Managing Processes & Tools],[Project Name (Optional)],[Project Number (Optional)],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([Client / Customer Name],[Leadership / Management Issues],[Management or Project Management Issue],[Project Management Issues],[Project Managing Processes & Tools],[Project Name (Optional)],[Project Number (Optional)],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)])
    ) p
) y
