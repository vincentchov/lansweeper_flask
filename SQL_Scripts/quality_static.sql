...
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [Continouous Improvement / ACE],[Quality & Continuous Improvement / ACE],[Quality-Related Issues],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([Continouous Improvement / ACE],[Quality & Continuous Improvement / ACE],[Quality-Related Issues],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)])
    ) p
) y
