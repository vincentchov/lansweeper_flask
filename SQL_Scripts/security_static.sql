...
AS (
    ...
    WHERE htblticketcustomfield.fieldid
        IN (15,45,50,51,52,53,135,136,137,138)
    ORDER BY [TicketID],[FieldID]
)
SELECT *
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [Access System / Badging],[Has the issue been addressed or fixed?],[Location of Issue],[Security Issue Type],[Security],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([Access System / Badging],[Has the issue been addressed or fixed?],[Location of Issue],[Security Issue Type],[Security],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)])
    ) p
) y
