...
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [Location of Issue],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([Location of Issue],[Was this an Escape? (Min of 2 bus.days, +$5,000 loss, or Export Violation)])
    ) p
) y
