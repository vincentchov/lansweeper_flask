...
AS (
    ...
    WHERE htbltickettypes.typename
        LIKE 'Service / Maintenance Request (Non-IT)'
...
FROM to_join
LEFT JOIN (
SELECT DISTINCT TicketID AS TicketID_2, [Location of Issue],[Service / Maintenance Request Types]
    FROM (
    ...
    PIVOT (
        MAX(FieldData)
        FOR FieldName IN ([Location of Issue],[Service / Maintenance Request Types])
    ) p
) y
